...
* Signály `sig_u`, `sig_d`, `sig_r`, `sig_l`  určují vstupní směrové povely (`Up, Down, Right, Left`). Vidíme, že změna směru v paměti proběhne okamžitě, ale fyzický pohyb v datech nastane až s následným pulsem `sig_ce_game`.  Signál `sig_snake_out (x, y)` prokazuje správnou reakci na směrové povely. Při povelu doprava se začne zvyšovat hodnota `x`, při povelu doleva naopak snižovat. Hodnota `y` je po celou dobu konstantní. Při pohybu nahoru se hodnota `y` začne snižovat a při pohybu dolu zase zvyšovat. Hodnota `x` je v těchto případech konstantní.
